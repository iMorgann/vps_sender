...
/**
 * Lightweight recipient-domain probe: MX lookup → TCP port 25 only.
 * No SPF/DMARC — those aren't needed for recipient reachability.
 * Uses a shorter default timeout (3 s) to keep bulk scans fast.
 */
async function probeRecipientDomain(domain, proxy = null, timeout = 3000) {
  let mx = null;
  try {
    const recs = await dns.resolveMx(domain);
    if (recs && recs.length) {
  ...
  if (!mx) return { domain, mx: null, open: false };

  try {
    const sock = await openRawSocket(mx, 25, proxy, timeout);
    return await new Promise(resolve => {
      let buf = "";
      const timer = setTimeout(() => { sock.destroy(); resolve({ domain, mx, open: false }); }, timeout);
      sock.once("error", () => { clearTimeout(timer); resolve({ domain, mx, open: false }); });
      sock.on("data", chunk => {
        buf += chunk.toString();
        if (/^\d{3}[ -]/m.test(buf)) {
          clearTimeout(timer);
          try { sock.write("QUIT\r\n"); } catch { /**/ }
          sock.destroy();
          resolve({ domain, mx, open: parseInt(buf.slice(0, 3), 10) === 220 });
        }
      });
    });
  } catch {
    return { domain, mx, open: false };
  }
}

/**
 * Scan recipient domains at high concurrency using the lightweight prober.
 * Default concurrency=100; results are pushed to onProgress as they arrive.
 */
async function scanRecipientDomains(domains, proxy = null, concurrency = 100, onProgress = null, cancelToken = null) {
  const results = [];
  let idx = 0;
  async function worker() {
    while (true) {
      ...
      const myIdx = idx++;
      if (myIdx >= domains.length) break;
      const r = await probeRecipientDomain(domains[myIdx], proxy);
      results.push(r);
      if (onProgress) onProgress(results.length, domains.length, r);
    }
  }
...
module.exports = { probePort25, checkDomainAuth, scanDomains, probeRecipientDomain, scanRecipientDomains };
