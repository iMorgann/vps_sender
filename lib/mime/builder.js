...
function buildMailOptions(opts) {
  ...
  const safeName   = fromName
    ? fromName.replace(/[\r\n\0]/g, "").replace(/\\/g, "\\\\").replace(/"/g, '\\"').slice(0, 100)
    : "";
  const displayFrom = safeName ? `"${safeName}" <${fromEmail}>` : fromEmail;

  const fromDomain    = (fromEmail && fromEmail.includes("@") ? fromEmail.split("@")[1] : "") || "localhost";
  ...
  const safeDomain  = /^[a-zA-Z0-9.\-]+$/.test(fromDomain) ? fromDomain : "localhost";
  const unsubMailto = `<mailto:unsubscribe@${safeDomain}?subject=unsubscribe>`;
  if (vars.unsubscribe_url) {
    headers["List-Unsubscribe"]      = `<${vars.unsubscribe_url}>, ${unsubMailto}`;
