 ...
 */
async function openRawSocket(host, port, proxy = null, timeoutMs = 15000, localAddress = null) {
  if (!proxy) {
    return new Promise((resolve, reject) => {
      const connOpts = localAddress ? { host, port, localAddress } : { host, port };
      const s = net.createConnection(connOpts);
      s.setNoDelay(true);
