...
async function runCampaign(params) {
  ...
  if (!csv) {
    ...
    ownCsv = true;
    if (needsHeader) csv.write("email,from_email,subject,smtp,status,error,tls\n");
  }
  ...
  const sendOne = async (item, i) => {
    ...
    try {
      const result = await withRetry(
        () => deliverEmail(item.email, item.mxHost, mime, fromEmail, HELO_HOST, { proxy: PROXY, tlsRejectUnauthorized: TLS_REJECT, sendingIp: SENDING_IP }),
        {
          maxRetries:   3,
