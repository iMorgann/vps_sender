...
const { buildMime, interpolate } = require("../mime/builder");
const { deliverEmail }           = require("../delivery/smtp-client");
...
async function runCampaign(params) {
  ...
  const GREYLIST_WAIT = config.greylistWait ?? 60_000;
  ...
  if (cancelToken.cancelled) {
    // Write any reachable-but-cancelled items as skipped so resume doesn't lose them
  ...
  let csv    = csvStream;
  if (!csv) {
    const mode = alreadySent.size ? "a" : "w";
    csv    = fs.createWriteStream(RESULTS_FILE, { flags: mode });
    ownCsv = true;
    if (!alreadySent.size) csv.write("email,from_email,subject,smtp,status,error,tls\n");
  }
  ...
  const sendOne = async (item, i) => {
    ...
    const fromEmail = smtp.fromEmail || smtp.fromemail || "";
    const fromDomain = fromEmail.split("@")[1] || item.domain;
    ...
    let mime;
    try {
      mime = await buildMime({ to: item.email, fromEmail, fromName, subject, html, attachments, config, templateVars });
    } catch (err) {
      stats.failed++;
      completedSends++;
    ...
    try {
      const result = await withRetry(
        () => deliverEmail(item.email, item.mxHost, mime, fromEmail, HELO_HOST, { proxy: PROXY, tlsRejectUnauthorized: TLS_REJECT }),
        {
          maxRetries:   3,
